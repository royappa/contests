	...
	while (numLeft > 0) // While some packages not reached
	{
		...
		while (i < numLeft) // For each unreached packet
		{
			...
			for (int p = 0; p < nbr; p++)
			{
				...
				int jBest;
				int best = 100000, bestCount = 10000;
				for (int j = p; j < nbr; j++)
				{
					...
					else if (S[vn][dst[pi]] == best && count[vn] < bestCount)
					{
						best = S[vn][dst[pi]];
						jBest = j;
						bestCount= count[vn];
			...
			int j = 0;
			bool moved = false;
			for (int t = 0+rand()%5; t < nbr && S[adjL[vpi][t]][dst[pi]] <= S[adjL[vpi][0]][dst[pi]]+5; t++)
			{
				int vn = adjL[vpi][j];
