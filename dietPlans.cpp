///////
////// C++ CPP SORTING SORT COMPARISON COMPAR COMPARE ALGORITHM COMPARISON FUNCTION
/////// WORKS
  #include <vector>
  #include <list>
  #include <map>
  ...
  class Compar
  {
  	...
  	Compar(string _d, vector<int> _p, vector<int> _c, vector<int> _f)
  	{
  		diet = _d;
  		p = _p;
  		c = _c;
  	...
  	bool operator()(int a, int b)
  	{
  		...
  		{
  			char ch = diet[i];
  			if (ch == 'T' && p[a]*5+c[a]*5+f[a]*9 != p[b]*5+c[b]*5+f[b]*9)
  				return (p[a]*5+c[a]*5+f[a]*9 < p[b]*5+c[b]*5+f[b]*9);
  			if (ch == 't' && p[a]*5+c[a]*5+f[a]*9 != p[b]*5+c[b]*5+f[b]*9)
  				return (p[a]*5+c[a]*5+f[a]*9 > p[b]*5+c[b]*5+f[b]*9);
  			if (ch == 'P' && p[a] != p[b]) return p[a] < p[b];
  			if (ch == 'p' && p[a] != p[b]) return p[a] > p[b];
  			if (ch == 'C' && c[a] != c[b]) return c[a] < c[b];
  			if (ch == 'c' && c[a] != c[b]) return c[a] > c[b];
  			if (ch == 'F' && f[a] != f[b]) return f[a] < f[b];
  			if (ch == 'f' && f[a] != f[b]) return f[a] > f[b];
  		}
  		return a > b;
  	}
  ...
  class HealthFood {
  public:
  	vector <int> selectMeals(vector <int>, vector <int>, vector <int>, vector <string>);
  ...
  vector <int> HealthFood::selectMeals(vector <int> protein, vector <int> carbs, vector <int> fat, vector <string> dietPlans) {
  	...
  	for (int i = 0; i < dietPlans.size(); i++)
  	{
  		...
  		stable_sort(meals.begin(), meals.end(), Compar(dietPlans[i], protein, carbs, fat));
  		best.push_back(meals[n-1]);		
  	}
  	return best;
  }
...
selectMeals
Parameters:
vector <int>, vector <int>, vector <int>, vector <string>
Returns:
vector <int>
Method signature:
