...
int f(int booth, int time) // booth is in original ordering
{
	...
	int r = p[booth] == time; //I get a fab prize for being here

	// check all other booths I can visit
	int m = 0;
	for (multimap<int,pi>::iterator it = sched.lower_bound(time+1); it != sched.end() && it->first >= time+t[booth][it->second.second]; it++)
	{
		if (it->second.second == booth)
			continue;
		m = max(m, f(it->second.second, it->first));
	}
...
main ()
{
	...
	{
		fin >> p[i];
		sched.insert(make_pair(p[i], make_pair(p[i], i)));
	}
